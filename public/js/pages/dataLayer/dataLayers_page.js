...
var pageTask={
  ...
  deleteItem:function(itemId){
    ...
    var msg='Are you sure you want to delete selected data layer?';
...
deleteItemForReal:function(itemId){
  var self=this;
  var url = '/datalayer/' + itemId + '/delete?_method=DELETE';
  $.ajax(url, {
      type: 'POST'
  ...
  fillItems:function(items){
    ...
    for(var i=0;i<items.length;i++){
      ...
      html+='       <a href="/map/preview?layers='+item.id+'" >';
      if(item.thumbnail){
      ...
      html+='       </li>';
      // if(
      //    (app.identity.id==item.OwnerUser.id) ||
      //    ( (app.identity.isAdministrator || app.identity.isPowerUser  || app.identity.isDataManager ||  app.identity.isDataAnalyst) && item._userHasPermission_EditSchema)
      //    )
      if( app.identity.isAdministrator ||
         (app.identity.id==item.OwnerUser.id) ||
          ( (app.identity.isPowerUser || app.identity.isDataManager ||  app.identity.isDataAnalyst) && item._userHasPermission_EditSchema))    
      {
          html+='       <li>';
          html+='          <a href="/datalayer/'+item.id+'?dataType='+item.dataType+'"><i class="glyphicon glyphicon-edit"></i></a>'; 
           html+='       </li>';  
       }
       html+='       <li>';
      //  if( ( app.identity.id==item.OwnerUser.id)
      //      || ( ((app.identity.isAdministrator || app.identity.isPowerUser || app.identity.isDataManager ||  app.identity.isDataAnalyst) && item._userHasOwnerPermission))
      //       || (item.OwnerUser.parent== app.identity.id )
      //   )
      if(app.identity.isAdministrator || ( app.identity.id==item.OwnerUser.id)
                                      || (item.OwnerUser.parent== app.identity.id ))
        {
          // html+='       <form method="POST" action="/datalayer/'+item.id+'/delete?_method=DELETE">';
  ...
  fillStatistics_datasettypes:function(statistics,pagination){
    var self=this;
    var html='';
    if(!statistics)
    return;
  ...
  getFromUrl:function(url,skipPushState){
    ...
    self.last_url=url;
    self.showWaiting();
    $.ajax( {    url: '/datalayers'+url+'&format=json', dataType: 'json', success: function (data) {
      if(!skipPushState){
        history.pushState({url:url}, document.title, url);
      }
        if(data){
