...
LayerStyleTab.prototype.populateUniqueValueRenderer=function(){
 ...
 for(var key in items){
   ...
   htm+='<td>' +item['label'] || item['value'] +'</td>' ;
...
// var items=this.renderer.getUniqueValueInfos();
 ...
 this.rendererContent.find('#cmdAddAllUniqueValues').click(function(){
   ...
   for (var i = 0; fields && i < fields.length; i++) {
       var fld = fields[i];
       var fldName_ = fld.name;
       var title = fld.alias || fldName_;
       fieldsDic[fldName_] = fld;
   ...
   for(var i=0;i< valueArray.length;i++){
     if(shapeType=='Polygon'|| shapeType=='MultiPolygon'){
       self.renderer.addValue(valueArray[i].value,valueArray[i].label+'',StyleFactory.randomStyle({
         randomStrokeColor:false,
         strokeColor:defSym.getStroke().getColor(),
         strokeWidth:defSym.getStroke().getWidth()}));  
     }else{
       self.renderer.addValue(valueArray[i].value,valueArray[i].label+'',StyleFactory.randomStyle({randomStrokeColor:true,strokeWidth:defSym.getStroke().getWidth()}));
     }
   }
   self.populateRendererPanel(); 
 });
